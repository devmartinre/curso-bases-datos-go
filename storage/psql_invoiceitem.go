...
const (
	psqlMigrateInvoiceItem = `CREATE TABLE IF NOT EXISTS invoice_items (
		...
		FOREIGN KEY (invoice_header_id) REFERENCES invoice_headers (id) ON UPDATE 
...
// PsqlInvoiceItem used to work with postgres - product
...
func (p *PsqlInvoiceItem) Migrate() error {
	...
	fmt.Println("migration of invoice item executed successfully")
	return nil
}
